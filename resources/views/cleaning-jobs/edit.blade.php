<x-layouts.app>
    <div class="max-w-2xl mx-auto py-10 px-4">
        ...
        <div class="mb-6">
            <a href="{{ route('cleaning-jobs.show', $cleaningJob) }}"
        ...
        <div class="bg-white rounded-md border border-slate-200 overflow-hidden mb-4">
            ...
            <div class="bg-slate-50 border-b border-slate-200 px-6 py-5">
                <h1 class="text-lg font-semibold text-slate-800">Edit Cleaning Job</h1>
                <p class="text-sm text-slate-500">{{ $cleaningJob->property->house }}, {{ $cleaningJob->property->street }}, {{ $cleaningJob->property->postcode }}</p>
            </div>

            <form method="POST" action="{{ route('cleaning-jobs.update', $cleaningJob) }}">
                @csrf
                @method('PUT')

                <div class="divide-y divide-slate-100">
                    <x-cleaning-job-form-fields :cleaning-job="$cleaningJob" :property-services="$propertyServices" :cleaners="$cleaners" />
                </div>
