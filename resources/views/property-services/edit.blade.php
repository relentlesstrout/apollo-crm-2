<x-layouts.app>
    <div class="max-w-2xl mx-auto py-10 px-4">
        ...
        <div class="mb-6">
            <a href="{{ route('properties.show', $propertyService->property) }}"
               class="text-sm text-slate-500 hover:text-slate-700 transition-colors duration-150 flex items-center gap-1">
                ...
                Back to {{ $propertyService->property->house }}, {{ $propertyService->property->street }}
            </a>
        </div>
        ...
        <div class="bg-white rounded-md border border-slate-200 overflow-hidden mb-4">
            ...
            <div class="bg-slate-50 border-b border-slate-200 px-6 py-5">
                <h1 class="text-lg font-semibold text-slate-800">Edit Service</h1>
                <p class="text-sm text-slate-500">{{ $propertyService->service->name }}</p>
            </div>

            <form method="POST" action="{{ route('property-services.update', $propertyService) }}">
                @csrf
                @method('PUT')

                <div class="divide-y divide-slate-100">
                    <x-property-service-form-fields :property-service="$propertyService" :services="$services" />
                </div>
